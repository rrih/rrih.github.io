...
const url = (path) => new URL(/^https?:/.test(path) || path.startsWith('/atlas/') ? path : `/atlas${path}`, base).href
const stableUrl = value => value.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/')
...
const responseInfo = async ({ url, cacheName }) => {
  const response = cacheName
    ? await (await caches.open(cacheName)).match(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'))
...
const waitCached = (path, cacheName = modelCache) =>
  page.waitForFunction(
    async ({ url, cacheName }) => !!(await (await caches.open(cacheName)).match(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'))),
...
const showModel = async (id, name) => {
  ...
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), `${name}`)
...
try {
  ...
  await page.evaluate(
    async ({ cacheName, url }) => {
      ...
      await (await caches.open(cacheName)).put(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'), response)
    },
  ...
  await page.addInitScript((targetUrl) => {
    if (location.pathname !== '/atlas/' || new URLSearchParams(location.hash.slice(1)).get('pokemon') !== '500')
      return
  ...
  assert(result.initialPrecacheCount > 0 && result.initialPrecacheCount < 250)
  for (const locale of locales) {
    assert(precachePaths.has(`/atlas/locales/manifests/${locale}.webmanifest`), locale)
    assert(precachePaths.has(`/atlas/locales/ui/${locale}.json`), `UI dictionary ${locale}`)
  }
  assert.equal([...precachePaths].filter((path) => path.startsWith('/atlas/locales/catalog/')).length, 12)
  assert(!precachePaths.has('/atlas/artwork/500.webp'))
  ...
  result.onlineArtwork = await readResponse('/atlas/artwork/500.webp')
  assert.equal(result.onlineArtwork.status, 200)
  assert(result.onlineArtwork.bytes > 0)
  await waitCached('/atlas/artwork/500.webp', artworkCache)
  ...
  for (const id of modelIds.filter((id) => id !== 6))
    assert(retained[modelCache].includes(stableUrl(modelUrl(id))), `Missing model ${id}`)
  ...
  await page.evaluate(
    async ({ entries, date }) => {
      for (const { cacheName, url } of entries) {
        ...
        const response = await cache.match(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'))
        if (!response) throw new Error(`Cannot age missing entry ${url}`)
        const headers = new Headers(response.headers)
        headers.set('Date', date)
        await cache.put(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'), new Response(await response.arrayBuffer(), { status: response.status, headers }))
        if ((await cache.match(url.replace('/atlas/models/', '/models/').replace('/atlas/artwork/', '/artwork/'))).headers.get('Date') !== date) throw new Error('Date header was not aged')
      }
    },
    {
      entries: [
        { cacheName: modelCache, url: modelUrl(1) },
        { cacheName: artworkCache, url: url('/atlas/artwork/500.webp') },
      ],
      date: result.agedDate,
    },
  )
  result.offlineIcon = await readResponse('/atlas/icon.svg')
  await page.evaluate(async () => {
    const previews = await caches.open('atlas-previews-v1')
    await previews.delete('/atlas/previews/500.webp')
    await previews.delete('/atlas/previews/forms/charizard-mega-x.webp')
  })
  ...
  result.uncachedPreviewArtworkFallback = await readResponse('/atlas/previews/500.webp')
  result.uncachedFormPreviewIconFallback = await readResponse('/atlas/previews/forms/charizard-mega-x.webp')
  assert.deepEqual(result.uncachedPreviewArtworkFallback, result.onlineArtwork)
  assert.deepEqual(result.uncachedFormPreviewIconFallback, result.offlineIcon)
  assert.equal(await page.evaluate(async () => !!(await (await caches.open('atlas-previews-v1')).match('/atlas/previews/500.webp'))), false, 'Fallback never replaces the real preview in its cache')
  ...
  result.offlineArtwork = await readResponse('/atlas/artwork/500.webp')
  ...
  result.offlineManifests = await page.evaluate(async (locales) => {
    const records = []
    for (const locale of locales) {
      const response = await fetch(`/atlas/locales/manifests/${locale}.webmanifest`, { cache: 'no-store' })
      const manifest = await response.json()
      if (
        response.status !== 200 ||
        manifest.lang !== locale ||
        !manifest.start_url.startsWith('/atlas/') ||
        new URL(manifest.start_url, location.origin).searchParams.get('lang') !== locale ||
  ...
  result.reconnectedPreview = await readResponse('/atlas/previews/500.webp')
  assert.equal(result.reconnectedPreview.status, 200)
  assert.notEqual(result.reconnectedPreview.sha256, result.onlineArtwork.sha256)
  await waitCached('/atlas/previews/500.webp', 'atlas-previews-v1')
  passed('Uncached offline previews use saved artwork/icon without poisoning the preview cache; reconnect restores the real preview')
  ...
  await worker.evaluate(`(() => {
    ...
    self.__atlasQuotaProbe = { url: ${JSON.stringify(stableUrl(quotaUrl))}, attempts: 0, throws: 0, restored: false, recoveryComplete: false };
    let quotaSettled;
  ...
  await page.reload()
  await page.getByText('LIVE VIEW', { exact: true }).waitFor({ timeout: 90_000 })
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), 'Charizard')
