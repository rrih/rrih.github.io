import { urlLocale } from './helpers.mjs'
import { appBase } from './helpers.mjs'
import assert from 'node:assert/strict'
import { mkdir, readFile, writeFile } from 'node:fs/promises'
import { chromium, expect } from './helpers.mjs'
...
async function state(page, locale, status) {
  ...
  await expect(page.locator('.header nav a').first()).toHaveText(dictionaries[locale].Explore)
...
async function englishFallback(page, locale) {
  ...
  await expect(page.locator('.specimen-heading bdi')).toHaveText(english.name)
...
async function localizedEntry(page, locale, sourceLocale = locale) {
  ...
  await expect(page.locator('.specimen-heading bdi')).toHaveText(entry.name)
...
try {
  await scenario('ui-dictionary-startup-retry-preserves-html', {}, async (page) => {
    let blocked = true, requests = 0
    await page.route('**/atlas/locales/ui/fr.json', route => {
      requests++
      return blocked ? route.abort('failed') : route.continue()
    })
    await page.goto(`${base}/fr/pokemon/6/`)
    await expect(page.getByRole('status')).toContainText('The language could not be loaded. Try again.')
    await expect(page.locator('#root h1')).toContainText('Dracaufeu')
    assert.equal(await page.locator('canvas').count(), 0, 'Readable SSG remains until the language can load')
    assert.equal(await page.evaluate(() => localStorage.getItem('atlas-language')), null)
    blocked = false
    await page.getByRole('button', { name: 'Retry translations', exact: true }).click()
    await localizedEntry(page, 'fr')
    assert(requests >= 2, 'Dictionary is fetched again after failure')
    return { initialHtmlPreserved: true, retryRecovered: true, requests }
  })
  await scenario('ui-language-change-is-transactional', {}, async (page) => {
    await page.goto(`${base}/pokemon/6/?lang=en`)
    await state(page, 'en', 'ready')
    await page.locator('.language-button').click()
    await page.locator('[data-language=en]').click()
    const before = await page.evaluate(() => ({ preference: localStorage.getItem('atlas-language'), scenes: localStorage.getItem('atlas-scenes-v1'), url: location.href }))
    let blocked = true, requests = 0
    await page.route('**/atlas/locales/ui/fr.json', route => {
      requests++
      return blocked ? route.fulfill({ status: 503, body: 'Temporary unavailable' }) : route.continue()
    })
    await page.locator('.language-button').click()
    await page.locator('[data-language=fr]').click()
    await expect(page.locator('.language-dialog [role=status]')).toContainText('The language could not be loaded. Try again.')
    await state(page, 'en', 'ready')
    assert.deepEqual(await page.evaluate(() => ({ preference: localStorage.getItem('atlas-language'), scenes: localStorage.getItem('atlas-scenes-v1'), url: location.href })), before)
    blocked = false
    await page.getByRole('button', { name: 'Retry translations', exact: true }).click()
    await localizedEntry(page, 'fr')
    assert.equal(await page.evaluate(() => localStorage.getItem('atlas-language')), 'fr')
    assert(requests >= 2)
    return { preservesPreferenceSceneAndUrl: true, retryRecovered: true, requests }
  })
  ...
  await scenario('language-selection-without-storage', {}, async (page, context) => {
    ...
    for (const locale of ['ja', 'fr']) {
      ...
      await localizedEntry(page, locale)
      assert.equal(urlLocale(page.url()), locale)
      await page.locator('.language-button').click()
      await expect(page.locator(`[data-language="${locale}"]`)).toHaveAttribute('aria-current', 'true')
      await expect(
  ...
  for (const test of notFoundCases) {
    await scenario(test.name, { locale: test.browserLocale }, async (page, context) => {
      ...
      const url = new URL('/atlas/404.html', base)
      ...
      await expect(page.locator('main a')).toHaveAttribute('href', `/atlas/?lang=${test.expected}`)
