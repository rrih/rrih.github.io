...
import { appBase, manifestStart } from './helpers.mjs'
...
model500.url = `/atlas${model500.url}`
...
const catalogSources = await Promise.all(
  catalogFiles.map(async (name) => {
    ...
    return {
      path: `/atlas/locales/catalog/${name}`,
      bytes: bytes.length,
...
const observe = (target) => {
  ...
  target.on('response', (response) => {
    if (new URL(response.url()).pathname.startsWith('/atlas/locales/catalog/')) {
...
const description = async (target, uiLocale, entryLocale) => {
  ...
  assert.equal(await target.locator('.specimen-heading bdi').innerText(), expected.name)
  if (uiLocale === 'ar') {
...
try {
  ...
  await page.waitForFunction(
    ...
    [...expectedCatalogPaths, '/atlas/models/home/6.glb'],
    { timeout: 120_000 },
  )
  ...
  const precache = await page.evaluate(async () => {
    ...
    for (const name of names) {
      ...
      for (const request of await cache.keys()) {
        ...
        const path = new URL(request.url).pathname
        if (!path.startsWith('/atlas/locales/catalog/')) continue
  ...
  assert.equal(precache.urls.filter(url => new URL(url).pathname.startsWith('/atlas/locales/ui/')).length, 49, 'All 49 UI dictionaries are available for offline first use')
  ...
  assert(!(await cachedPaths(page)).includes(model500.url.replace('/atlas/models/', '/models/')), 'Model 500 must never have been cached')
  result.precache = {
  ...
  assert.equal(result.manifest.start_url, manifestStart('en'))
  assert.equal(result.manifest.scope, '/atlas/')
  assert.equal(result.manifest.id, '/atlas/')
  ...
  for (const code of ['fr', 'ja', 'ar']) {
    ...
    assert.equal(urlLocale(page.url()), code)
  ...
  for (const code of ['fr', 'ja', 'ja-Hrkt']) {
    assert(
      catalogResponses.some(
        (response) =>
          response.path === `/atlas/locales/catalog/${code}.json` &&
  ...
  assert(!(await cachedPaths(page)).includes(model500.url.replace('/atlas/models/', '/models/')))
  await firstCdp.detach()
  const savedLanguageManifest = await page.evaluate(async () => (await fetch(document.querySelector('link[rel=manifest]').href)).json())
  await page.close()

  phase = 'offline-saved-ar-launch'
  page = await context.newPage()
  observe(page)
  const launchUrl = new URL(savedLanguageManifest.start_url, base).href
  assert.equal(new URL(launchUrl).searchParams.get('lang'), 'ar')
  assert.equal(new URL(launchUrl).hash, '')
  // Launch the manifest published for the chosen language in a fresh document.
  ...
  assert(!(await cachedPaths(page)).includes(model500.url.replace('/atlas/models/', '/models/')), 'Model 500 must still be absent before selection')
  await page.getByRole('searchbox').fill('500')
  await page
    .getByRole('link', { name: dictionaries.ar['View {name}'].replace('{name}', emboar.name), exact: true })
    .click()
  ...
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), emboar.name)
  ...
  await page.waitForFunction(
    async (path) => {
      const response = await (await caches.open('atlas-models-v1')).match(path.replace('/atlas/models/', '/models/'))
