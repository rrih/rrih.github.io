...
try {
  ...
  await change('ja')
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), 'リザードン')
  ...
  await ready(page, 'en')
  assert.equal(await page.evaluate(() => localStorage.getItem('atlas-language')), 'ja', 'Explicit English route does not erase saved preference')
  await page.goto(`${base}/?lang=fr`)
  await ready(page, 'fr')
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), 'Dracaufeu')
  ...
  for (const code of codes) {
    ...
    assert.equal(await page.locator('.header nav a').first().innerText(), t.Explore)
    const desktop = await bounds(page)
    const manifest = await page.evaluate(async code => (await fetch(document.querySelector('link[rel=manifest]')?.href || `/atlas/locales/manifests/${code}.webmanifest`)).json(), code)
    assert.equal(manifest.lang, code)
    assert.equal(manifest.id, '/atlas/')
    assert.equal(manifest.start_url, manifestStart(code))
    assert.equal(manifest.scope, '/atlas/')
    await page.locator('.header-actions .icon-button').click()
    assert((await page.locator('dialog').innerText()).includes(t['How to explore']))
    await page.keyboard.press('Escape')
    await page.locator('.detail-tabs').getByRole('button', { name: t.Studio, exact: true }).click()
    assert.equal(await page.getByLabel(t['Render quality'], { exact: true }).count(), 1)
    await page.locator('.detail-tabs').getByRole('button', { name: t.Overview, exact: true }).click()
    ...
    await page.locator('.mobile-toolbar button').first().click()
  ...
  await page.locator('.pokemon-card').click()
  await ready(page, 'ja')
  assert.equal(await page.locator('.specimen-heading bdi').innerText(), 'モモワロウ')
  assert.equal(await page.locator('.description').getAttribute('lang'), 'ja')
  assert.equal(await page.locator('.entry-language').count(), 0)
  assert.deepEqual(errors, [])
  await writeFile(`${output}/verification.json`, JSON.stringify({ base, codes, results, errors, cameraPreserved: true, storageAndShare: true, japaneseCompletion: true }, null, 2))
