...
const readRuntimeArtwork = async () => {
  const response = await fetch('/atlas/artwork/500.webp')
...
try {
  ...
  assert(
    initialPrecacheCount > 0 && initialPrecacheCount < 250,
    'First installation must precache fewer than 250 assets, including all language manifests',
  )
  assert(
    !initialPrecacheUrls.some((url) => new URL(url).pathname === '/atlas/artwork/500.webp'),
  ...
  await page.getByRole('link', { name: 'View Emboar', exact: true }).click()
  await page
  ...
  await page.getByRole('link', { name: 'View Charizard', exact: true }).click()
  await page.getByText('LIVE VIEW', { exact: true }).waitFor({ timeout: 60_000 })
  ...
  await page.waitForFunction(async () => {
    ...
    return (await cache.keys()).some((request) => request.url.includes('/models/shiny-home/6.glb'))
  })
  ...
  const result = {
    ...
    runtimeArtworkPath: '/atlas/artwork/500.webp',
    onlineRuntimeArtwork,
    offlineRuntimeArtwork,
    offlineReload: true,
    offline3D: true,
