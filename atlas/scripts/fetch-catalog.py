...
def build(cache: Path, verified_at: str) -> tuple[list[dict], dict]:
    ...
    descriptions = {}
    group_order = {int(row["id"]): int(row["order"]) for row in read_csv(cache / "version_groups.csv")}
    versions = {
        int(row["id"]): (row["identifier"], (group_order[int(row["version_group_id"])], int(row["id"])))
        for row in read_csv(cache / "versions.csv")
    }
    version_names = {
        int(row["version_id"]): clean_text(row["name"])
        for row in read_csv(cache / "version_names.csv") if row["local_language_id"] == "9"
    }
    for row in read_csv(cache / "pokemon_species_flavor_text.csv"):
        species_id = int(row["species_id"])
        if row["language_id"] == "9" and clean_text(row["flavor_text"]):
            if species_id not in descriptions or versions[int(row["version_id"])][1] > versions[int(descriptions[species_id]["version_id"])][1]:
    ...
    for row in species:
        ...
        version_id = int(descriptions[species_id]["version_id"])
        entry = {
            "id": species_id,
            "slug": row["identifier"],
            "name": english["name"],
            ...
            "sourceLabel": f"PokéAPI · {version_names[version_id]}",
            "sourceVersion": versions[version_id][0],
    ...
    manifest = {
        ...
        "flavorTextSelection": "Latest nonempty English source text by version_groups.order, then version ID; whitespace normalized",
...
def main() -> None:
    ...
    catalog, manifest = build(arguments.cache, arguments.verified_at)
