...
--cache directory to reproduce the output without network access. Upstream text
is preserved. Reviewed Atlas completions are applied separately and labelled;
--source-only emits only the source records and their provenance.
"""
...
def build(cache: Path, verified_at: str) -> tuple[dict, dict, dict]:
    ...
    version_names = {
        (int(row["version_id"]), int(row["local_language_id"])): clean_text(row["name"])
        for row in rows["version_names.csv"]
    }
    ...
    for language in sorted(rows["languages.csv"], key=lambda row: int(row["id"])):
        ...
        for species_id in species_ids:
            ...
            if species_id in descriptions[language_id]:
                selected = descriptions[language_id][species_id]
                version_id = selected["versionId"]
                label_language = {11: 1, 14: 7}.get(language_id, language_id)
                version_label = version_names.get((version_id, language_id)) or version_names.get((version_id, label_language)) or version_names.get((version_id, 9))
                assert version_label, f"Missing readable edition label: {locale}/{version_id}"
                record.update({
                    "description": selected["text"],
                    "descriptionKind": "source-entry",
                    "sourceURL": f"https://pokeapi.co/api/v2/pokemon-species/{species_id}/",
                    "sourceLabel": f"PokéAPI · {version_label}",
                    "sourceVersion": versions[version_id]["identifier"],
    ...
    for locale, catalog in catalogs.items():
        for species_id, record in catalog["species"].items():
            ...
            assert set(record) <= {"name", "genus", "description", "descriptionKind", "sourceURL", "sourceLabel", "sourceVersion", "verifiedAt"}
    ...
    manifest = {
        ...
        "textCleaning": "Source extraction collapses whitespace and removes discretionary soft hyphens with their line wrapping; source text is not translated or rewritten.",
        "missingData": "Source extraction omits missing fields. The optional completion phase adds separately attributed reviewed summaries or localized numeric facts; it never labels these as game text.",
        "coverageScope": "The coverage field reports pinned upstream availability before completions; completion records and final output hashes describe the published result.",
        "coverage": coverage,
        "search": {
...
def main() -> None:
    ...
    parser.add_argument("--verified-at", default=date.today().isoformat(), help="Date the pinned source snapshot was checked; not a page modification date")
    parser.add_argument("--source-only", action="store_true", help="Omit reviewed Atlas completions and generate upstream-only records")
    ...
    date.fromisoformat(arguments.verified_at)
    catalogs, search, manifest = build(arguments.cache, arguments.verified_at)
    if not arguments.source_only:
        from catalog_completion import complete_catalogs
        manifest["completion"] = complete_catalogs(catalogs, arguments.verified_at)
    ...
    for locale in catalogs:
        ...
        records = catalogs[locale]["species"].values()
        counts = ", ".join(f"{field}={sum(bool(record.get(field)) for record in records)} (source {detail['count']})" for field, detail in fields.items())
