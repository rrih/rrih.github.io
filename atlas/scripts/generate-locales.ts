...
import { assetPath, atlasPath } from '../src/routes'
...
mkdirSync('public/locales/ui', { recursive: true })
for (const [locale] of locales) {
  writeFileSync(`public/locales/ui/${locale}.json`, `${JSON.stringify(messages[locale])}\n`)
  writeFileSync(
    `${output}/${locale}.webmanifest`,
    `${JSON.stringify({
      name: 'Pokémon Atlas',
      short_name: 'Atlas',
      description: `Pokémon Atlas — ${messages[locale]['3D models']}`,
      lang: locale,
      dir: isRTL(locale) ? 'rtl' : 'ltr',
      id: '/atlas/',
      start_url: `${atlasPath({ kind: 'home' }, locale)}?lang=${locale}`,
      scope: '/atlas/',
      display: 'standalone',
      ...
      icons: [
        ...
        { src: assetPath('/icon-maskable.png'), sizes: '512x512', type: 'image/png', purpose: 'maskable' },
      ],
    })}\n`,
  )
}
...
const copy=words[language];document.documentElement.lang=language;document.documentElement.dir=['ar','he','fa','ur'].includes(language)?'rtl':'ltr';document.title=copy.title+' · Pokémon Atlas';document.querySelector('h1').textContent=copy.title;document.querySelector('p').textContent=copy.text;const link=document.querySelector('a');link.textContent=copy.link;link.href='/atlas/?lang='+encodeURIComponent(language);
</script></body></html>\n`,
)
