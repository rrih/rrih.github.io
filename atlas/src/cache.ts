...
export async function cacheOpenedView(modelUrl: string | string[] | undefined) {
  if (!navigator.serviceWorker?.controller || !navigator.onLine) return
  const artwork = Array.from(document.images)
    .map((image) => image.currentSrc || image.src)
    .filter((src) => {
      const url = new URL(src, location.href)
      return url.origin === location.origin && /^\/atlas\/artwork\/\d+\.webp$/.test(url.pathname)
    })
  const urls = [
    ...new Set([...(Array.isArray(modelUrl) ? modelUrl : modelUrl ? [modelUrl] : []), ...artwork]),
  ]
  let next = 0
  await Promise.all(
