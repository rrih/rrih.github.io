...
export const models = Object.fromEntries(
  Object.entries(modelData as Record<string, ModelEntry>).map(([id, model]) => [
    id,
    { ...model, url: assetPath(model.url), ...(model.shiny ? { shiny: assetPath(model.shiny) } : {}) },
  ]),
)
...
export const art = (id: number) => assetPath(`/artwork/${id}.webp`)
