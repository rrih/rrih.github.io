...
describe('complete UI and attributed catalogs', () => {
  test('all 49 UI dictionaries have every interface key and exact placeholders', () => {
    expect(locales.length).toBe(49)
    const keys = Object.keys(english).sort()
    expect(keys.length).toBeGreaterThanOrEqual(227)
  ...
  test('all source catalogs map every species and identify reviewed completions', () => {
    for (const code of [
      'en',
      'ja',
      'ja-Hrkt',
      ...
      if (code === 'ja') {
        ...
        expect(catalog.species['1025'].description.length).toBeGreaterThan(0)
        expect(catalog.species['1025'].descriptionKind).toBe('atlas-summary')
        expect(catalog.species['1025'].sourceURL).toBe('https://zukan.pokemon.co.jp/detail/1025')
      }
