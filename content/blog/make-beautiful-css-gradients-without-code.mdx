...
relatedTools: [
  ...
  },
  {
    ...
    "description": "Turn finished gradients into animated CSS effects",
...
### Copy-Paste Gradient Recipes for Common UI Blocks

If you are not sure where to start, pick the UI block first and then tune the colors in the
[Gradient Generator](/tools/gradient-generator). These recipes are intentionally simple so they
stay readable, fast, and easy to adapt.
...
Use this when the page needs a strong first impression. Keep the darkest stop behind the headline
area, then test white text over the final gradient.
...
Use this for one important call to action, not every button on the page. A gradient works best when
it signals priority.
...
Use this when a pricing tier, feature card, or dashboard panel needs emphasis without turning the
whole card into a bright block of color.

After choosing a recipe, open the [Color Picker](/tools/color-picker) to adjust brand colors and
the [CSS Animation Generator](/tools/animation-generator) if you want to test a subtle moving
gradient.
...
## Gradient Launch Checklist

Before publishing a gradient to a production page, run through this quick check:

1. **Contrast:** Place real headline and button text over the gradient, then confirm it remains
   readable at mobile and desktop sizes.
2. **Fallback color:** Add a solid background color before the gradient declaration so old browsers
   or email clients still show an acceptable surface.
3. **Brand fit:** Compare the gradient against your logo, product screenshots, and existing button
   colors. If it feels like a separate brand, reduce saturation or stay closer to your core palette.
4. **Responsiveness:** Test tall mobile screens and wide desktop sections. Radial and conic
   gradients can move their focal point in surprising ways when the container shape changes.
5. **Motion restraint:** If the gradient moves, keep it slow and provide a static alternative for
